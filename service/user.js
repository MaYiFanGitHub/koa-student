...
exports.selectAllUser = async ctx => {
  ...
  if (role_id != -1) {
    ...
    if (flagId === 0) {
      ...
      sql += ' and t_user.role_id = 2 or t_user.role_id = 3';
      countSql += ' and t_user.role_id = 2 or t_user.role_id = 3';
    }
  }
...
exports.addUser = async ctx => {
  ...
  if (role_id === 2 || role_id === 4) { // 添加教秘/教师身份
    let teacherObj= {
      ...
      college_id: ctx.session.userInfo.user_college_id || ctx.session.userInfo.teacher_college_id,
      teacher_title
    }
...
exports.addMoreTeacher = async ctx => {
  ...
  let userList1 = obj.map(user => {
    ...
    user.teacher_id = parseInt(
      ...
      .toString()
      .substr(6, 12);
    return [user.teacher_id, user.user_id, teacher_titles.pop(), ctx.session.userInfo.user_college_id || ctx.session.userInfo.teacher_college_id];
  });
...
exports.findDeanUser = async ctx => {
  let sql =
    'select * from t_user where role_id in (select role_id from t_role where role_name = "院长")';
  const result = await db.query(sql);
  ctx.success(result);
};
...
exports.findTeacherUser = async ctx => {
  let sql =
    'select t_user.user_name, t_teacher_info.teacher_id from t_user RIGHT JOIN t_teacher_info ON t_user.user_id = t_teacher_info.user_id where role_id in (select role_id from t_role where role_name = "教师")';
  const result = await db.query(sql);
  ctx.success(result);
};
...
exports.login = async ctx => {
  let params = ctx.request.body;
  let sql =
    'select t_role.*,t_user.user_id,t_user.role_id,t_user.user_name,t_user.user_username, t_user.college_id as user_college_id, t_teacher_info.teacher_id,t_teacher_info.college_id as teacher_college_id ,t_student_info.student_id,t_student_info.politics_status_id,t_student_info.politics_status_info_id,t_student_info.class_id from t_user left join t_role on t_user.role_id = t_role.role_id LEFT JOIN t_teacher_info ON t_user.user_id=t_teacher_info.user_id LEFT JOIN t_student_info ON t_user.user_id=t_student_info.user_id WHERE t_user.is_delete=0 and t_user.user_username = ? and t_user.user_password = ? and t_user.role_id = ?';
  const result = await db.query(sql, [
    params.user_username,
